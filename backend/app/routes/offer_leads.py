...
async def create_lead(lead: OfferLeadCreate = Body(...)):
    ...
    # Initialize is_read as False
    lead_dict["is_read"] = False
...
@router.patch("/{lead_id}/toggle-read", response_model=OfferLead, response_model_by_alias=True)
async def toggle_read(lead_id: str, is_read: bool = Body(embed=True), admin: str = Depends(get_current_admin)):
    db = get_database()
    if not ObjectId.is_valid(lead_id):
        raise HTTPException(status_code=400, detail="Invalid lead ID")
    
    updated = await db.offer_leads.find_one_and_update(
